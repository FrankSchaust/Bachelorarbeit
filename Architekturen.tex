...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure {Powervalue}{$unit$, $enemies$}
\LeftComment*{jeder Bonustyp beinhaltet einen Wert type, als Identifikator und einen Wert damage, der den Bonusschaden spezifiziert.}
	\ForAll {$bt \in unit.bonustype$}
		\If {$Anzahl\,der\,Gegner\,mit\,Bonustyp\,bt.type > 0$}
...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure {Armypowervalues}{$army$, $opposingarmy$}
	\ForAll {$unit \in army$}
		...
		\EndIf	
	\EndFor
\LeftComment{powervalue enthält die Werte für ground und air}
...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure {Decider}{$armya$, $armyb$}
\If {$\neg armya.fightable \land \neg armya.fightable$}
	\State \textbf{return} 'Remis' 
\EndIf
\If {$armya.fightable  \land \neg armyb.fightable$}
	\If {$armya.powervalue.ground * K \geq armyb.defense.ground \land armya.powervalue.air * K \geq armyb.defense.air$}
	...
	\EndIf
\EndIf
\If {$\neg armya.fightable \land armyb.fightable$}
\If {$armyb.powervalue.ground * K \geq armya.defense.ground \land armyb.powervalue.air * K \geq armya.defense.air$}
